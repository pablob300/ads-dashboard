...
export default function ClientDashboard({ client }: { client: Client }) {
  ...
  const fetchData = useCallback(async (start: string, end: string) => {
    ...
    try {
      const res = await fetch(
        `/api/clients/${client.id}/campaigns?startDate=${start}&endDate=${end}`
      );
      ...
      setSelectedCampaigns(new Set((json as CampaignData).campaigns.map((c) => c.id)));
    } catch (err) {
      setData(null);
      addToast(err instanceof Error ? err.message : "Erro ao carregar dados. Tente novamente.", "error");
    } finally {
      setLoading(false);
    }
  }, [client.id, addToast]);
  ...
  // ── dados do gráfico filtrados pelas campanhas selecionadas ──────────────
  const chartData = useMemo(() => {
    if (!data) return [];
    if (selectedCampaigns.size === data.campaigns.length) return data.dailyMetrics;

    // Aproxima métricas diárias proporcionalmente às campanhas selecionadas
    // (a API retorna métricas diárias agregadas, não por campanha)
    ...
    return data.dailyMetrics.map((d) => ({
      ...
      costBRL:     Math.round(d.costBRL * ratio * 100) / 100,
  ...
  const totals = useMemo(() => {
    ...
    const costBRL     = sel.reduce((s, c) => s + c.costBRL, 0);
        ...
        {payload.map((p) => (
          <div key={p.name} className="flex items-center justify-between gap-4">
            ...
            <span className="font-medium text-slate-800">
              {p.name === "Valor Gasto" ? fmtBRL(p.value)
                : p.name === "Impressões" || p.name === "Cliques" ? fmtNum(p.value)
                : p.value.toFixed(1)}
            </span>
          </div>
        ))}
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex gap-1 border-b border-slate-200">
        ...
        >
          <svg viewBox="0 0 24 24" className="w-4 h-4 shrink-0" fill={activeTab === "meta" ? "#1877F2" : "#94a3b8"}>
      ...
      {/* Conteúdo da aba Google (só renderiza se ativa) */}
      {activeTab === "google" && <>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {/* Busca + multiselect de campanhas */}
          ...
          >
            ...
            <span className="flex-1 text-left text-slate-600 truncate">
              {selectedCampaigns.size === 0
                ? "Nenhuma campanha"
                : selectedCampaigns.size === data?.campaigns.length
                ? "Todas as campanhas"
            ...
            <div className="absolute top-10 left-0 z-50 w-80 bg-white border border-slate-200 rounded-xl shadow-xl">
              ...
              <div className="p-2 border-t border-slate-100">
                <button
                  ...
                  className="w-full text-center text-xs text-slate-500 hover:text-slate-700 py-1 transition"
                >
                  Fechar
                </button>
              </div>
                    ...
                    tableCampaigns.map((c: CampaignMetric) => (
                      <tr key={c.id} className="hover:bg-slate-50 transition">
                        ...
                        <td className="px-4 py-3 text-slate-700 tabular-nums">
                          {c.conversions > 0 ? fmtBRL(c.costPerConversion) : "—"}
                        </td>
                      </tr>
                    ))
                  )}
                ...
                {tableCampaigns.length > 0 && (
                  <tfoot>
                    <tr className="border-t-2 border-slate-200 bg-slate-50">
                      ...
                      <td className="px-4 py-3 text-xs font-semibold text-slate-800 tabular-nums">
                        {totals.conversions > 0 ? fmtBRL(totals.costBRL / totals.conversions) : "—"}
                      </td>
                    </tr>
                  </tfoot>
                )}
        ...
        <div key={p.name} className="flex items-center justify-between gap-4">
          ...
          <span className="font-medium text-slate-800">
            {p.name === "Valor Gasto"
              ...
              : p.name === "Impressões" || p.name === "Cliques"
