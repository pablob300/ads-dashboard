...
- **Confirmado em produção** (deploy `45b52b2`, banco local estava vazio então a verificação foi feita direto em prod, que é onde o Palazzo Murano tem dados reais):
  - **Palazzo Murano** (01–17/08/2026): KPI **Resultados = 31** (era 0), campanha `[LEADS] [MENSAGENS] WhatsApp` com a sub-linha `↳ Conversas iniciadas`, Custo/Result. R$ 60,97, funil batendo. **Só a aba Meta Ads aparece.** Na listagem, "1 conta" com tag Meta (antes "0 contas")
  - **GWM Navesa** (dois canais): as duas abas aparecem, `?tab=meta` funciona, Resultados = 70.194, e a mesma conta mistura tipos traduzidos corretamente por campanha (`↳ Alcance`, `↳ Conversas iniciadas`, `↳ ThruPlays`)
  - **A v21 aceita o campo `results`** — o fallback do `fetchInsights` não precisou entrar em ação, mas fica como rede de segurança para o próximo bump de versão
- **Limitação inerente ao que foi pedido (não é bug):** numa conta que mistura objetivos, o TOTAL da coluna Resultados soma unidades diferentes — uma campanha de Alcance conta alcance como "resultado", uma de mensagens conta conversas. Foi exatamente o pedido ("quantidade de resultados independente do tipo") e é o mesmo comportamento do gerenciador, que também não totaliza essa coluna de forma significativa. O número **por campanha** é o correto, e agora vem rotulado com o tipo
- **Correção de sobra (commit `54bc313`):** o subtítulo do dashboard renderizava a contagem do Google incondicionalmente — Palazzo Murano lia "0 contas Google · 1 conta Meta" em produção. Agora cada canal só aparece se tiver conta, mesmo padrão do `shared-dashboard.tsx`
- **`npm run build` local voltou a funcionar** nesta sessão (compilou em ~8s, exit 0), apesar do gotcha #17. O `node_modules` deve ter sido hidratado pelo OneDrive desde a Sessão 17 — o gotcha continua válido como causa provável se voltar a falhar

### Sessão 19 — 2026-08-27 (Tabela de campanhas: coluna CPC, nova ordem e colunas arrastáveis)
- **Pedido:** (1) colunas na ordem `Campanha · Valor Investido · Impressões · Cliques · CTR · CPC · Resultados · Custo/Result.` — CPC é coluna nova, e Custo/Result. passa a vir depois de Resultados; (2) poder reorganizar as colunas clicando e arrastando, tanto na versão de edição quanto no link compartilhado com o cliente
- **Ponto de partida:** a tabela existia **quatro vezes** com markup quase idêntico — `client-dashboard.tsx` (Google), `meta-dashboard.tsx` (Meta) e as duas de `shared-dashboard.tsx`. Qualquer ajuste de coluna exigia editar os quatro blocos, então o primeiro passo foi extrair um componente único
- **Decisões do usuário:** persistência **só no `localStorage`** (sem tocar em banco/API — a ordem que a agência monta não chega ao cliente, cada navegador tem a sua); ordem **independente por canal**; coluna `Campanha` **fixa** na primeira posição
- **Módulos novos:**
  - `src/lib/campaign-columns.ts` — registro das colunas (rótulo, `title` por extenso, renderizador de célula e de total), `DEFAULT_COLUMN_ORDER`, adaptadores `rowsFromGoogle`/`rowsFromMeta`/`totalsFrom*`, e o store de persistência. Os helpers `fmtBRL`/`fmtNum`/`fmtPct`, que estavam **duplicados nos três dashboards**, passaram a morar aqui
  - `src/components/campaigns/CampaignTable.tsx` — a tabela, com o drag-and-drop
- **Shape normalizado (`CampaignRow`):** o Google chama o investimento de `costBRL` e o Meta de `spend`, e só o Meta tem `resultType` (a sub-linha `↳ Conversas iniciadas`). Os adaptadores achatam essa diferença para a tabela não saber de qual canal veio o dado
- **CPC e Custo/Result. são sempre derivados nos adaptadores** (`cost/clicks` e `cost/conversions`), nunca lidos do campo da API — é o que mantém linha e TOTAL usando a mesma fórmula. `MetaCampaignMetric` já trazia `cpc`, mas o `CampaignMetric` do Google **não tem esse campo**; derivar cobriu os dois sem mexer nas libs de API
- **Sem dependência nova de DnD:** o projeto não tem nenhuma (`lucide-react` está instalado mas nunca foi usado — o repo faz SVG inline), e o HTML5 Drag and Drop resolveu em ~50 linhas
- **`useSyncExternalStore` em vez de `useEffect` + `setState`:** ler `localStorage` no render quebraria a hidratação, e lê-lo num efeito somaria mais uma violação de `react-hooks/set-state-in-effect` (regra que o repo já viola em 5 pontos pré-existentes). O store serve `DEFAULT_COLUMN_ORDER` no SSR e troca pela ordem salva ao hidratar. **De brinde:** duas tabelas do mesmo canal na mesma tela ficam sincronizadas sozinhas
- **Bug pego na verificação em navegador (que o `tsc` não pegaria):** o `onDrop` lia a coluna de origem do state `dragging`. Como `dragstart` e `drop` podem cair no mesmo tick, o handler enxergava o valor defasado do closure e o drop virava no-op. Passou a ler de `e.dataTransfer.getData("text/plain")` — que já era preenchido no `dragstart` por causa do Firefox. O state ficou só para o realce visual
- **`loadColumnOrder` é defensiva:** descarta ids desconhecidos e **acrescenta ao fim** os que faltarem. Sem isso, uma coluna adicionada no futuro ficaria invisível para quem já tem ordem gravada. `try/catch` em volta do `localStorage` (aba anônima lança)
- **Fallback de toque:** o HTML5 DnD não funciona em tela sensível ao toque, e o link compartilhado tem grande chance de ser aberto no celular. Cada cabeçalho tem setas `‹ ›` (sempre visíveis abaixo de `sm`, no hover acima) que movem a coluna uma posição
- **Verificação:** `npx tsc --noEmit` limpo; `npx eslint` sem nenhum achado nos arquivos novos (os que sobram nos dashboards são todos pré-existentes). O componente foi exercitado no navegador numa página descartável, já removida: as 8 colunas na ordem pedida, contas conferidas (5400/3600 = R$ 1,50; 8400/90 = R$ 93,33; 0 cliques e 0 resultados → `—`), drop de `Custo/Result.` sobre `Impressões` levando a coluna para depois de `Valor Investido`, ordem sobrevivendo ao reload, TOTAL acompanhando, tabela do mesmo canal sincronizando, chave do Meta intacta, botão "Restaurar ordem padrão" aparecendo/sumindo, e ordem corrompida à mão sendo reconciliada para as 8 colunas
- **Não verificado:** as telas internas (`client-dashboard`/`meta-dashboard`) renderizando com dados reais — o banco local está vazio e as tentativas de semear um cliente e de abrir a rota foram bloqueadas pelo classificador de permissões. Cobertura indireta: `tsc` limpo, as quatro substituições conferidas no código, e o componente validado ao vivo
- **Ajuste imediato (mesma sessao):** a listagem passou a **esconder campanha sem impressao no periodo** e a **ordenar por nome**, igual nos dois canais. Antes, o Google ja nao trazia campanha zerada (a GAQL filtra `impressions > 0 AND cost_micros > 0`) e vinha ordenado por gasto decrescente; o Meta nao filtrava nada e vinha na ordem da Graph API. A regra virou a funcao `listed()` em `campaign-columns.ts`, aplicada dentro de `rowsFromGoogle`/`rowsFromMeta` — os quatro pontos que montam a tabela nao tem como divergir
- **Ordenacao:** `localeCompare(name, "pt-BR", { sensitivity: "base", numeric: true })`. `numeric` poe "Campanha 2" antes de "Campanha 10"; `sensitivity: "base"` ignora caixa e acento, senao "Otica" cairia depois de "zebra" e minusculas depois de todas as maiusculas
- **TOTAL passou a sair das linhas exibidas** (`totalsFromRows`), em vez do `totals` da selecao inteira que os dashboards ja calculam. Motivo concreto: uma campanha do Meta pode ter **0 impressoes no periodo e ainda assim resultados**, atribuidos por janela de 7 dias — escondendo a linha e mantendo o total antigo, a coluna Resultados nao fecharia com o proprio TOTAL, o que parece defeito. Os KPIs no topo continuam com o total da selecao inteira, que responde outra pergunta. A prop `totals` do `CampaignTable` foi removida junto, e com ela os adaptadores `totalsFromGoogle`/`totalsFromMeta`
- **Titulo unificado:** a tabela do Google dizia "Campanhas no periodo" e a do Meta "Campanhas com impressoes no periodo". Com o comportamento agora identico, as quatro dizem "Campanhas com impressoes no periodo"
- **Verificacao:** `tsc` e `eslint` limpos (so os achados pre-existentes). No navegador, com dados montados de proposito: das 6 campanhas do Google a de 0 impressoes sumiu (contador 5), ordem saiu `alfa marca, Campanha 2, Campanha 10, Otica pesquisa, zebra final` — os tres casos dificeis de uma vez (caixa, numero, acento) — e o TOTAL fechou com a soma da coluna (R$ 7.200, 190.000 impressoes, 3.300 cliques, 28 resultados). No Meta, a campanha com 0 impressoes **e 9 resultados** foi escondida e o TOTAL ficou 160, sem os 9. Arraste de coluna continua funcionando e nao afeta a ordem das linhas
- **Sem migration** — nenhuma mudança de schema

### Sessao 20 — 2026-08-27 (Controle de Orcamento no link compartilhado, somente leitura)
- **Pedido:** o cliente poder ver o Controle de Orcamento pelo link compartilhado, **sem** criar nem editar, e a entrada so aparecer quando ja houver orcamento cadastrado
- **Decisoes do usuario:** pagina separada `/share/[token]/budget`, alcancada por um botao no cabecalho do relatorio (espelha o fluxo interno), e **com seletor de mes** — o cliente consegue rever meses fechados, que costuma ser o dado mais util
- **O problema real nao era a UI, era o calculo.** O GET de `/api/clients/[id]/budget` tinha ~120 linhas de agregacao de gasto (duas APIs de anuncio, gross-up de imposto do Meta, resolucao de campanhas por sub-relatorio e canal) presas dentro da rota autenticada. Copiar isso para a rota publica seria a forma mais facil de o numero que o cliente ve divergir do nosso
- **`src/lib/budget-server.ts` (novo)** — `buildBudgetResponse()` com todo esse calculo, mais `clientHasBudget()`. As duas rotas passam a consumir a mesma funcao. A rota autenticada caiu de **243 para 129 linhas** e o POST ficou intacto. Fica separado de `budget.ts` de proposito: aquele e importado por componentes client e nao pode puxar Prisma nem as APIs de anuncio
- **Resolucao de conexao sem sessao:** a rota publica nao tem `userId`, entao as conexoes sao buscadas por `connectionId` da propria conta (`findUnique`), padrao que `share/[token]/campaigns` ja usava. A conta sempre vem de um cliente ja validado — pelo token no link, pelo `userId` na rota interna —, entao o escopo continua garantido. A rota autenticada foi unificada nesse mesmo caminho
- **`src/components/budget/BudgetSummaryGrid.tsx` (novo)** — os cards agrupados por canal, extraidos de `budget-control.tsx`: e a parte puramente de leitura, comum as duas telas (inclui o `BudgetGridSkeleton`). A ordenacao de canais passou a usar `CHANNEL_KEYS` do registro de canais em vez de um `CHANNEL_ORDER` local duplicado. O formulario de cadastro continua so na versao interna
- **Somente leitura de verdade, nao so na UI:** a rota publica exporta **apenas GET**. Um POST nela devolve **405**, verificado. O POST continua exclusivo de `/api/clients/[id]/budget`, que exige sessao
- **`subReports` e `availableChannels` nao vao na resposta publica** — so servem para montar os selects do formulario interno, e o link compartilhado nao tem formulario
- **Visibilidade da entrada:** `clientHasBudget()` (conta verbas do cliente em qualquer mes) decide tanto o botao no cabecalho quanto o acesso a propria pagina. Devolve `false` se a tabela `budget_entries` nao existir, entao **migration pendente vira ausencia do botao**, nunca um erro na cara do cliente — diferente da tela interna, que mostra o SQL de propósito
- **Sem toast na tela compartilhada:** o `(shared)/layout.tsx` nao monta o ToastProvider, e o cliente nao tem o que fazer com um erro de token da API de anuncio. Falha de canal vira um aviso ambar discreto, e os cards que carregaram continuam visiveis
- **Verificacao:** `tsc` limpo, `eslint` sem achado novo, `npm run build` exit 0 com as duas rotas registradas. Token invalido devolve "Link invalido ou expirado" na pagina e 404 na API; POST na rota publica devolve 405. `BudgetSummaryGrid` conferido no navegador nas quatro configuracoes (dois canais lado a lado, um canal em largura total, vazio, skeleton) — os quatro percentuais bateram exatamente com os dados montados (79%, 106%, 87%, 30%)
- **Nao verificado:** os contadores GSAP chegando ao valor final, e a tela com dados reais. O banco local esta vazio, e a automacao mantem a aba em segundo plano, o que estrangula o `requestAnimationFrame` e congela o GSAP — os quatro cards pararam na mesma fracao de 77,08%, assinatura de timeline pausada e nao de valor errado. `BudgetSummaryCard` nao foi tocado nesta sessao
- **Sem migration** — nenhuma mudanca de schema
